...
import (
	...
	"github.com/matrixorigin/matrixone-operator/pkg/api"
...
var (
	Log = klogr.New().WithName("mo-oeprator")
)
...
// GlogWrite serves as a bridge between the standard log package and the glog package
...
// InitLogs initializes logs the way we want to kubernetes
...
func (l *MLogWithFormat) Infof(format string) {
	l.Info(fmt.Sprintf(format))
}
